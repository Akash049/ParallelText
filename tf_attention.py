...
# Loading Data
spa = pd.read_csv('corpus.txt', header=None, sep='\n')
spa.columns = ['Content']
...
train = spa.iloc[0 : int(0.8 * spa.shape[0]), ]
test = spa.iloc[int(0.8 * spa.shape[0]) : , ]

# Adding a column to test dataset
test['Predicted_Spanish'] = ''
...
#Storing as lists
tr_en_text  = train['English'].tolist()  
tr_sp_text  = train['Spanish'].tolist()
ts_en_text  = test['English'].tolist() 
ts_sp_text = test['Spanish'].tolist()

#Checking List sizes
assert(len(tr_en_text) == len(tr_sp_text))
assert(len(ts_en_text) == len(ts_sp_text))

#Appending start and end tokens to spanish sentences
...
batch_size = 256
hidden_size = 96
en_timesteps, sp_timesteps = 100, 100
...
# Function 4	
def train_model(full_model, en_seq, sp_seq, batch_size, n_epochs=10):
...
def infer_nmt(encoder_model, decoder_model, test_en_seq, en_vsize, sp_vsize):
    ...
    for i in range(20):
        ...
        test_sp_seq = sents2sequences(sp_tokenizer, [sp_index2word[dec_ind]], sp_vsize)
        
        test_sp_onehot_seq = np.expand_dims(to_categorical(test_sp_seq, num_classes=sp_vsize), 1)
        attention_weights.append((dec_ind, attention))

        sp_text += sp_index2word[dec_ind] + ' '
...
#Defining voabulary size
en_vsize = max(en_tokenizer.index_word.keys()) + 1
sp_vsize = max(sp_tokenizer.index_word.keys()) + 1
...
#Training the model
train_model(full_model, en_seq, sp_seq, batch_size, n_epochs)

#Saving the model
full_model.save('attention_nmt.h5')

""" Inferring with trained model """
for i in range(len(ts_en_text)):
    ...
    test.iloc[i,2] = test_sp
	#print('English: ', ts_en_text[i])
    #print('Spanish: ', test_sp)
    #print('-'*50)

# Saving the test dataset
test.to_csv('Predicted_Value.csv', index = False)
